...
public class Puzzle {
   ...
   public void newPuzzle(int numToGuess) {
      ...
      boolean[][] hardcodedIsShown = new boolean [GameBoard.GRID_SIZE][GameBoard.GRID_SIZE];

      // Copy from hardcoded masks
      for (int row = 0; row < GameBoard.GRID_SIZE; ++row) {
         for (int col = 0; col < GameBoard.GRID_SIZE; ++col) {
        	 
        	 boolean no = rd.nextBoolean();
        	 	if(no == false & counter < 12 & Math.random()< 0.25) {
        	 hardcodedIsShown[row][col] = no;
        	  ++counter; 
        	 	}
        	 	else {
        	 		hardcodedIsShown[row][col] = true;
        	 	}
